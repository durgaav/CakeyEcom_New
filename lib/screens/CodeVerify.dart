  ...
  Future<void> addUsertoDb() async {
    ...
    try {
      http.Response response = await http.post(
        ...
        body: jsonEncode(<String, dynamic>{"PhoneNumber": int.parse(num)}),
      );
      //check status code...
      if (response.statusCode == 200) {
        var map = jsonDecode(response.body);
        //Checking msg....(reg / login)
        if (map['message'].toString().toLowerCase() ==
  ...
  Future<void> onTimeLogin(String phonenumber) async {
    ...
    try {
      http.Response response = await http.post(
        ...
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode(
            <String, dynamic>{"PhoneNumber": int.parse(phonenumber)}),
      );
                  ...
                  Container(
                    ...
                    child: CustomRaisedButton(
                      onPressed: () async {
                        ...
                        if (otpControl.text.isEmpty ||
                            otpControl.text.length < 6) {
                          Functions()
                              .showSnackMsg(context, "Invalid OTP Code!", true);
                        } else {
